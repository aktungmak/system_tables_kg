from pyspark.sql.functions import format_string, lit, url_encode, split_part
from pyspark.sql.column import Column

dbx_prefix = "http://www.databricks.com/ontology/systemTables/"
# TODO add angle brackets to all IRIs
...
def user(user_id: str) -> Column:
    return format_string(dbx_prefix + "user/%s", user_id)
...
def cluster(workspace_id: str, cluster_id: str) -> Column:
    return format_string(dbx_prefix + "cluster/%s/%s", workspace_id, cluster_id)


def job(workspace_id: str, job_id: str) -> Column:
    return format_string(dbx_prefix + "job/%s/%s", workspace_id, job_id)


def task(workspace_id: str, job_id: str, task_key: str) -> Column:
    return format_string(dbx_prefix + "task/%s/%s/%s", workspace_id, job_id, task_key)


def pipeline(workspace_id: str, pipeline_id: str) -> Column:
    return format_string(dbx_prefix + "pipeline/%s/%s", workspace_id, pipeline_id)


def workspace(workspace_id: str) -> Column:
    return format_string(dbx_prefix + "workspace/%s", workspace_id)


def notebook(workspace_id: str, notebook_id: str) -> Column:
    return format_string(dbx_prefix + "notebook/%s/%s", workspace_id, notebook_id)


def pred(name: str) -> str:
    return dbx_prefix + name
