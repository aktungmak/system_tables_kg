...
mappings = {
    ...
    "pipelines": Mapping(
        source=scd2_latest(
            "system.lakeflow.pipelines", "change_time", "workspace_id", "pipeline_id"
        ),
    ...
    "query_to_table": Mapping(
        source=spark.table("system.access.table_lineage").filter(
            col("entity_metadata.sql_query_id").isNotNull()
        ),
        subject_map=iri.query("workspace_id", "statement_id"),
        predicate_object_maps=[
            (
                iri.pred("queryReadTable"),
                when(
                    col("source_table_name").isNotNull(),
                    iri.table(
                        "source_table_catalog",
    ...
    "pipeline_to_table": Mapping(
        source=spark.table("system.access.table_lineage").filter(
            col("entity_metadata.dlt_pipeline_info").isNotNull()
        ),
        subject_map=iri.pipeline(
            "workspace_id", "entity_metadata.dlt_pipeline_info.dlt_pipeline_id"
        ),
        predicate_object_maps=[
            (
                iri.pred("pipelineReadTable"),
                when(
                    col("source_table_name").isNotNull(),
                    iri.table(
                        "source_table_catalog",
...
mapped_names = [
    mapping.to_temporary_view(spark, name) for name, mapping in mappings.items()
]
